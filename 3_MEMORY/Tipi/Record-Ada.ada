...
procedure Record_Demo is
   ...
   -- Record per la data di nascita (deve essere dichiarato per primo)
   ...
   -- Record per i dati anagrafici
   type Dati_Anagrafici_Type is record
      Cognome       : String(1..20);
      Nome          : String(1..20);
      Data_Nascita  : Data_Nascita_Type;
   end record;

   -- Record per i dati lavorativi
   type Dati_Lavorativi_Type is record
      Dipartimento   : String(1..15);
      Stipendio      : Float;
      Anni_Servizio  : Integer;
   end record;

   -- Record completo Dipendente
